...
import { VendorBanner } from './types';
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Shop[]> => {
      ...
      if (isDev) {
        ...
        return [...mockShops]
          .sort(
            (a, b) => b.followersCount * b.rating - a.followersCount * a.rating
          )
          .slice(0, limit);
      }
...
// Product queries
export const useShopsProducts = (params?: {
  shopId?: string;
  category?: string;
  featured?: boolean;
  limit?: number;
      ...
      if (isDev) {
        ...
        if (params?.shopId) {
          filteredProducts = filteredProducts.filter(
            product => product.shop.id === params.shopId
          );
        }
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Product[]> => {
      ...
      const response = await fetch(
        `/api/products/personalized/${userId}?limit=${limit}`
      );
...
export const useVendorBanners = (limit?: number) => {
  return useQuery({
    ...
    queryFn: async (): Promise<VendorBanner[]> => {
      ...
      // Production API call
      const response = await fetch(
        `/api/vendor-banners${limit ? `?limit=${limit}` : ''}`
      );
