  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async (): Promise<ScanEventsStats> => {
      const response = await fetch(
        `/api/admin/scan-events/stats?days=${dateRange}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('authToken')}`,
          },
        ...
        <CardContent>
          <div className='grid grid-cols-1 md:grid-cols-5 gap-4'>
            ...
            >
              ...
              <option value='player'>
                {t('admin.scanEvents.playerScans')}
              </option>
              <option value='vendor'>
                {t('admin.scanEvents.vendorScans')}
              </option>
            </select>
                ...
                <thead>
                  <tr className='border-b'>
                    <th className='text-left p-3'>
                      {t('admin.scanEvents.timestamp')}
                    </th>
                    <th className='text-left p-3'>
                      {t('admin.scanEvents.ticketId')}
                    </th>
                    <th className='text-left p-3'>
                      {t('admin.scanEvents.result')}
                    </th>
                    <th className='text-left p-3'>
                      {t('admin.scanEvents.scannedBy')}
                    </th>
                    <th className='text-left p-3'>
                      {t('admin.scanEvents.device')}
                    </th>
                    <th className='text-left p-3'>
                      {t('admin.scanEvents.location')}
                    </th>
                  </tr>
                </thead>
                <tbody>
