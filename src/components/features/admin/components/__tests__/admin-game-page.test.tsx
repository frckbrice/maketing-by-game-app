...
jest.mock('@/lib/api/currencyService', () => ({
  currencyService: {
    formatCurrency: jest.fn(
      (amount: number, currency: string) => `$${amount.toFixed(2)}`
    ),
  },
}));
...
const mockUseGames = require('@/hooks/useApi')
  .useGames as jest.MockedFunction<any>;
const mockUseUpdateGame = require('@/hooks/useApi')
  .useUpdateGame as jest.MockedFunction<any>;
const mockUseDeleteGame = require('@/hooks/useApi')
  .useDeleteGame as jest.MockedFunction<any>;
...
describe('AdminGamesPage', () => {
  ...
  describe('Games Display', () => {
    it('displays game information correctly', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Mega Lottery')).toBeInTheDocument();
        expect(
          screen.getByText('The biggest lottery game with massive prizes')
        ).toBeInTheDocument();
  ...
  describe('Filtering and Search', () => {
    it('displays search input', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByPlaceholderText('Search games by title...')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Empty States', () => {
    it('displays appropriate message when no games exist', () => {
      ...
      const emptyMessages = screen.getAllByText(
        /No games found|Get started|Try adjusting/
      );
