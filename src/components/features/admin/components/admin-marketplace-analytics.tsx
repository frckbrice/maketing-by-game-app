...
export const AdminMarketplaceAnalytics: React.FC = () => {
  ...
  // Data queries
  const { data: stats, isLoading: statsLoading, refetch: refetchStats } = useMarketplaceStats();
  const { data: topShops, isLoading: shopsLoading } = useTopShopsPerformance(5);
  const { data: topProducts, isLoading: productsLoading } = useTopProductsPerformance(5);
  const { data: trends, isLoading: trendsLoading } = useMarketplaceTrends(selectedPeriod);
  ...
  if (!stats) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 dark:text-gray-400">{t('admin.marketplace.noData')}</p>
      </div>
    );
  }
              ...
              <div>
                ...
                <div className="flex items-center mt-2">
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    {formatNumber(stats.totalFollows)} {t('admin.marketplace.follows')}
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center mt-2">
                  ...
                  <span className="text-sm text-yellow-600 dark:text-yellow-400">
                    {stats.averageShopRating.toFixed(1)} {t('admin.marketplace.avgRating')}
                  </span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('admin.marketplace.last30Days')}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-80">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('admin.marketplace.dailyOrders')}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('admin.marketplace.bestPerformingShops')}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {shopsLoading ? (
                ...
                topShops?.map((shop, index) => (
                  <div key={shop.shopId} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800">
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center">
                        <span className="text-white font-bold text-sm">#{index + 1}</span>
                      </div>
                      <div>
                        <p className="font-medium text-gray-900 dark:text-white">{shop.shopName}</p>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('admin.marketplace.bestSellingProducts')}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {productsLoading ? (
                ...
                topProducts?.map((product, index) => (
                  <div key={product.productId} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800">
                    <div className="flex items-center space-x-3">
