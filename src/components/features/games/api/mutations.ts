...
export const useJoinGame = () => {
  ...
  return useMutation({
    mutationFn: async ({ gameId, ticketCount = 1 }: { gameId: string; ticketCount?: number }) => {
      ...
      queryClient.setQueryData(QUERY_KEYS.games, (oldData: any) => {
        ...
        return {
          ...
          data: oldData.data.map((game: any) => 
            game.id === gameId 
              ? { ...game, currentParticipants: game.currentParticipants + ticketCount }
              : game
          )
        };
      });
    ...
    onSuccess: (data) => {
      ...
      toast.success(`Successfully joined game! Purchased ${data.ticketCount} ticket(s).`);
    },
  });
};
...
export const useShareGame = () => {
  return useMutation({
    ...
    onSuccess: (data) => {
      const message = data.platform === 'copy' 
