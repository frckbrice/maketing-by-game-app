...
const mockT = jest.fn((key: string, options?: any) => {
    ...
    if (options) {
        return translations[key].replace(/\{\{(\w+)\}\}/g, (match: string, key: string) => options[key] || match);
    }
...
describe('GameCard', () => {
    ...
    describe('Rendering', () => {
        it('renders game card with basic information', () => {
            ...
            expect(screen.getByText('iPhone 15 Pro Max Giveaway')).toBeInTheDocument();
    ...
    describe('Status Badges', () => {
        ...
        it('shows hot badge when participation is over 75%', () => {
            const hotGame = { ...mockGame, currentParticipants: 180, maxParticipants: 200 }; // 90%
        ...
        it('shows filling fast badge when participation is over 90%', () => {
            const almostFullGame = { ...mockGame, currentParticipants: 190, maxParticipants: 200 }; // 95%
        ...
        it('applies correct styling for sponsored games', () => {
            ...
            expect(card).toHaveClass('border-yellow-300', 'dark:border-yellow-500', 'shadow-yellow-100', 'dark:shadow-yellow-900/20');
        });
    });

    describe('Company Information', () => {
        it('displays company info when provided', () => {
            render(<GameCard {...defaultProps} companyInfo={mockCompanyInfo} />);
        ...
        it('opens company website in new tab when clicked', () => {
            ...
            expect(mockOpen).toHaveBeenCalledWith('https://techcorp.com', '_blank', 'noopener,noreferrer');
        });
    ...
    describe('Timer Functionality', () => {
        ...
        it('shows expired message for ended games', () => {
            const expiredGame = { ...mockGame, endDate: Date.now() - 24 * 60 * 60 * 1000 }; // 1 day ago
        ...
        it('shows hours and minutes for games ending soon', () => {
            const soonGame = { ...mockGame, endDate: Date.now() + 2 * 60 * 60 * 1000 }; // 2 hours from now
    ...
    describe('Progress and Game States', () => {
        it('applies correct progress bar colors based on participation', () => {
            ...
            // Hot game (90%)
            const hotGame = { ...mockGame, currentParticipants: 180, maxParticipants: 200 };
            ...
            // Almost full (95%)
            const almostFullGame = { ...mockGame, currentParticipants: 190, maxParticipants: 200 };
        ...
        it('disables play button for expired games', () => {
            const expiredGame = { ...mockGame, endDate: Date.now() - 24 * 60 * 60 * 1000 };
        ...
        it('disables play button for full games', () => {
            const fullGame = { ...mockGame, currentParticipants: 200, maxParticipants: 200 };
    ...
    describe('Modal Interactions', () => {
        it('opens game detail modal when view details button is clicked', () => {
            ...
            expect(screen.getByText('Game: iPhone 15 Pro Max Giveaway')).toBeInTheDocument();
        });
        ...
        it('opens payment modal when play button is clicked', () => {
            ...
            expect(screen.getByText('Payment for: iPhone 15 Pro Max Giveaway')).toBeInTheDocument();
        });
    ...
    describe('Hover Effects', () => {
        ...
        it('shows hover overlay on image', () => {
            ...
            const imageContainer = document.querySelector('.relative.h-40.sm\\:h-48');
            const hoverOverlay = imageContainer?.querySelector('.absolute.inset-0.bg-black\\/20.opacity-0.group-hover\\:opacity-100');
        ...
        it('shows eye icon on image hover', () => {
            ...
            const eyeIcon = document.querySelector('.bg-white\\/90.dark\\:bg-gray-800\\/90.rounded-full.p-2.sm\\:p-3');
    ...
    describe('Accessibility', () => {
        it('has proper ARIA attributes', () => {
            ...
            const card = screen.getByRole('article');
            expect(card).toHaveAttribute('aria-label', 'Lottery Game: iPhone 15 Pro Max Giveaway');
        ...
        it('has proper button labels', () => {
            ...
            expect(playButton).toHaveAttribute('aria-label', 'Play iPhone 15 Pro Max Giveaway');
        });
        ...
        it('has proper scroll button labels', () => {
            ...
            const websiteButton = screen.getByLabelText('Visit TechCorp website');
            expect(websiteButton).toHaveAttribute('aria-label', 'Visit TechCorp website');
        });
    });
    ...
    describe('Edge Cases', () => {
        ...
        it('handles very old games gracefully', () => {
            const oldGame = { ...mockGame, createdAt: Date.now() - 48 * 60 * 60 * 1000 }; // 48 hours ago
    ...
    describe('Performance Optimizations', () => {
        it('uses React.memo to prevent unnecessary re-renders', () => {
            ...
            expect(screen.getByText('iPhone 15 Pro Max Giveaway')).toBe(initialRender);
        });
