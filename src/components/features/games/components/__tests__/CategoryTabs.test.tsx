...
describe('CategoryTabs', () => {
    ...
    describe('Rendering', () => {
        ...
        it('shows singular form for single game', () => {
            const singleGameCounts = { phones: 1, computers: 8, fashion: 12, electronics: 6 };
    ...
    describe('Selection and Interaction', () => {
        it('highlights selected category correctly', () => {
            ...
            expect(selectedTab).toHaveClass('bg-gradient-to-br', 'from-orange-500', 'to-red-500');
        });

        it('calls onCategoryChange when tab is clicked', () => {
            const mockOnCategoryChange = jest.fn();

            render(<CategoryTabs {...defaultProps} onCategoryChange={mockOnCategoryChange} />);
        ...
        it('updates selected category when prop changes', () => {
            ...
            expect(screen.getByText('Phones').closest('button')).toHaveAttribute('aria-selected', 'true');

            // Change to computers
            rerender(<CategoryTabs {...defaultProps} selectedCategory="computers" />);

            expect(screen.getByText('Computers').closest('button')).toHaveAttribute('aria-selected', 'true');
            expect(screen.getByText('Phones').closest('button')).toHaveAttribute('aria-selected', 'false');
        });
    });
    ...
    describe('Scrolling Functionality', () => {
        ...
        it('applies correct styling to scroll buttons', () => {
            ...
            expect(leftButton).toHaveClass('absolute', 'left-0', 'top-1/2', '-translate-y-1/2', 'z-10');
            expect(rightButton).toHaveClass('absolute', 'right-0', 'top-1/2', '-translate-y-1/2', 'z-10');
        });
    });
    ...
    describe('Visual Effects', () => {
        ...
        it('shows active indicator for selected category', () => {
            ...
            const selectedTab = screen.getByText('Phones').closest('button');
            const activeIndicator = selectedTab?.querySelector('.w-2.h-2.bg-white.rounded-full');
        ...
        it('applies hover effects correctly', () => {
            ...
            const hoverEffect = unselectedTab?.querySelector('.absolute.inset-0.bg-gradient-to-r.opacity-0.group-hover\\:opacity-10');
    ...
    describe('Category Description', () => {
        it('shows description for selected category', () => {
            ...
            expect(screen.getByText('Latest smartphones and mobile devices')).toBeInTheDocument();
        });

        it('does not show description for "all" category', () => {
            render(<CategoryTabs {...defaultProps} selectedCategory="all" />);

            expect(screen.queryByText('Latest smartphones and mobile devices')).not.toBeInTheDocument();
        });
        ...
        it('updates description when selection changes', () => {
            ...
            expect(screen.getByText('Laptops, desktops, and computing devices')).toBeInTheDocument();
        });

        it('applies correct styling to description container', () => {
            render(<CategoryTabs {...defaultProps} />);

            const descriptionContainer = screen.getByText('Latest smartphones and mobile devices').closest('div');
    ...
    describe('Accessibility', () => {
        it('has proper ARIA attributes', () => {
            ...
            expect(selectedTab).toHaveAttribute('aria-selected', 'true');
            expect(selectedTab).toHaveAttribute('aria-controls', 'category-phones-panel');
        });
    ...
    describe('Responsive Design', () => {
        it('applies responsive classes correctly', () => {
            ...
            const carousel = document.querySelector('.flex.overflow-x-auto.scrollbar-hide.space-x-4.px-16.py-4');
            expect(carousel).toBeInTheDocument();

            const tabs = screen.getAllByRole('tab');
            tabs.forEach(tab => {
                expect(tab).toHaveClass('flex-shrink-0', 'min-w-[120px]', 'max-w-[140px]');
            });
        });
    ...
    describe('Icon Handling', () => {
        ...
        it('shows fallback icon when no icon is provided', () => {
            const categoriesWithoutIcons = mockCategories.map(cat => ({ ...cat, icon: undefined }));

            render(<CategoryTabs {...defaultProps} categories={categoriesWithoutIcons} />);
        ...
        it('applies correct icon container styling', () => {
            ...
            const iconContainers = document.querySelectorAll('.mx-auto.rounded-full.flex.items-center.animate-bounce-gentle.justify-center.w-16.h-16');
    ...
    describe('Animation and Transitions', () => {
        it('applies animation classes correctly', () => {
            ...
            const iconContainer = selectedTab?.querySelector('.animate-bounce-gentle');
    ...
    describe('Edge Cases', () => {
        ...
        it('handles category without description', () => {
            const categoriesWithoutDescription = mockCategories.map(cat => ({ ...cat, description: undefined }));

            render(<CategoryTabs {...defaultProps} categories={categoriesWithoutDescription} />);
