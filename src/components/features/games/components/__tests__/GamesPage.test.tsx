...
const mockUseVendorApplication =
  require('../../api/queries').useVendorApplication;
...
const mockFeaturedGames = [
  {
    ...
    description: 'Amazing featured game',
  },
  {
    id: 'featured-2',
...
describe('GamesPage', () => {
  ...
  it('renders the games page with all sections', () => {
    ...
    // Check search input
    expect(
      screen.getByPlaceholderText('Search amazing prizes...')
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText('2 amazing prizes waiting for you')
    ).toBeInTheDocument();
  ...
  it('displays vendor application CTA for regular users', () => {
    ...
    expect(
      screen.getByText('Create Your Own Lottery Games')
    ).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  it('shows info toast for pending vendor application', () => {
    ...
    expect(toast.info).toHaveBeenCalledWith(
      'Your vendor application is already pending review.'
    );
  });
  ...
  it('handles error states', () => {
    ...
    expect(
      screen.getByText('Failed to load games. Please try again.')
    ).toBeInTheDocument();
  ...
  it('handles responsive design elements', () => {
    ...
    const searchContainer = screen
      .getByPlaceholderText('Search amazing prizes...')
      .closest('.max-w-lg');
