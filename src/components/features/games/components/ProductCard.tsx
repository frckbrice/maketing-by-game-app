...
import { ChatButton, FollowButton, LikeButton } from '@/components/features/social';
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mb-3">
          {product.shop && product.shop.shopId && (
            <>
              <FollowButton
                targetId={product.shop.shopId}
                targetType="SHOP"
                targetName={product.shop.shopName || 'Shop'}
                variant="compact"
                showCount={false}
              />
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="text-center">
            <div className="text-xs text-gray-500 dark:text-gray-400">{t('marketplace.likes')}</div>
            <div className="font-semibold text-sm text-gray-900 dark:text-white">{product.likeCount}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-500 dark:text-gray-400">{t('marketplace.played')}</div>
            <div className="font-semibold text-sm text-gray-900 dark:text-white">{product.playedCount}</div>
          </div>
          <div className="text-center">
            <div className="text-xs text-gray-500 dark:text-gray-400">{t('marketplace.rating')}</div>
        ...
        <div className="space-y-2 mt-auto">
          ...
          {showAddToCart && (
            ...
            >
              ...
              {product.status === 'OUT_OF_STOCK' ? t('marketplace.outOfStock') : t('marketplace.addToCart')}
              {cartQuantity > 0 && (
