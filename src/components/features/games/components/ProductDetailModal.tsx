...
export const ProductDetailModal = React.memo<ProductDetailModalProps>(function ProductDetailModal({
  product,
  relatedProducts = [],
  reviews = [],
  isOpen,
  ...
  const handleFollowShop = useCallback(async () => {
    ...
    setIsFollowing(true);
    try {
      await onFollowShop(product.shopId);
    } catch (error) {
  ...
  const isShopFollowed = followedShops.has(product.shopId);
            ...
            <div className="flex flex-col">
              ...
              <div className="mb-6">
                {product.stock > 0 ? (
                  <div className="flex items-center text-green-600 dark:text-green-400">
                    ...
                    <span className="text-sm">
                      {product.stock > 10 
                        ...
                        : t('marketplace.lowStock', { count: product.stock })
                      }
                    </span>
                  </div>
                ) : (
                  <div className="flex items-center text-red-600 dark:text-red-400">
                    ...
                    <span className="text-sm">{t('marketplace.outOfStock')}</span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                >
                  {isFollowing ? (
                    ...
                    <Heart className={`w-5 h-5 mr-2 ${isShopFollowed ? 'fill-current' : ''}`} />
                  )}
                  {isShopFollowed ? t('marketplace.following') : t('marketplace.followShop')}
                </button>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div className="text-center">
                  <div className="font-semibold text-gray-900 dark:text-white">{product.likesCount}</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">{t('marketplace.likes')}</div>
                </div>
                <div className="text-center">
                  <div className="font-semibold text-gray-900 dark:text-white">{product.playedCount}</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">{t('marketplace.played')}</div>
                </div>
                <div className="text-center">
                ...
                {relatedProducts.slice(0, 4).map(relatedProduct => (
                  <div key={relatedProduct.id} className="group cursor-pointer">
                    ...
                    <p className="text-orange-600 font-semibold">
                      {currencyService.formatCurrency(relatedProduct.price, relatedProduct.currency)}
                    </p>
                  </div>
                ))}
              </div>
