...
import { useMarkNotificationAsRead, useMarkAllNotificationsAsRead } from '../api/mutations';
import { NOTIFICATION_ICONS, PRIORITY_COLORS, type LiveNotification } from '../api/types';

export const LiveNotifications: React.FC<{ maxHeight?: string }> = ({ maxHeight = 'max-h-96' }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = realtimeService.subscribeToUserNotifications(user.id, (newNotifications) => {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      await markAllAsReadMutation.mutateAsync(unreadNotifications.map(n => n.id));
  ...
  const handleNotificationClick = (notification: LiveNotification) => {
    ...
    if (notification.data?.type === 'game-started' && notification.data.gameId) {
    ...
    } else if (notification.data?.type === 'order-confirmed' && notification.data.orderId) {
            ...
            <CardContent className="p-0">
              ...
              <div className="flex items-center justify-between p-4 border-b">
                <div className="flex items-center space-x-2">
                  <Bell className="w-5 h-5" />
                  <h3 className="font-semibold">{t('notifications.live.title')}</h3>
                  {unreadCount > 0 && (
              ...
              <div className={`${maxHeight} overflow-y-auto`}>
                {notifications.length > 0 ? (
                  ...
                  <div className="p-8 text-center text-gray-500">
                    ...
                    <p className="text-sm">{t('notifications.live.noNotifications')}</p>
                  </div>
                )}
              </div>
