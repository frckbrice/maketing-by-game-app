...
export const LikeButton: React.FC<LikeButtonProps> = ({
  ...
  const isLoading_mutation = likeMutation.isPending || unlikeMutation.isPending;
  ...
  if (isLoading) {
    return (
      <div className='flex items-center space-x-2'>
        <div
          className={`bg-gray-200 dark:bg-gray-700 rounded animate-pulse ${variant === 'icon' ? 'w-8 h-8' : 'w-12 h-6'}`}
        />
      </div>
    );
  }
    ...
    <div className='flex items-center space-x-2'>
      ...
      >
        ...
        {variant !== 'icon' && showCount && likesCount > 0 && (
          <span className='font-medium'>{formatLikesCount(likesCount)}</span>
        )}
      </Button>
