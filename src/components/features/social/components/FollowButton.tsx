...
export const FollowButton: React.FC<FollowButtonProps> = ({
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      // If custom onFollow callback is provided, use it
      if (onFollow) {
        await onFollow(targetId, targetType);
        return;
      }

      // Otherwise, use the default follow/unfollow mutations
  ...
  const getIcon = () => {
    if (targetType === 'SHOP') {
      return (
        <Heart
          className={`${variant === 'icon' ? 'h-4 w-4' : 'h-4 w-4 mr-2'} ${isFollowing ? 'fill-current' : ''}`}
        />
      );
    }
  ...
  if (isLoading) {
    return (
      <div className='flex items-center space-x-2'>
        <div
          className={`w-8 h-8 bg-gray-200 dark:bg-gray-700 rounded animate-pulse ${variant === 'icon' ? '' : 'w-20'}`}
        />
