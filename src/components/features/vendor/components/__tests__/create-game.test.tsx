...
describe('CreateGame', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    const { adminService } = jest.requireActual('@/lib/api/adminService');
    ...
    it('sets default form values', async () => {
      ...
      await waitFor(() => {
        ...
        const maxParticipantsInput =
          screen.getByLabelText('Max Participants *');
  ...
  describe('Prize Management', () => {
    it('adds new prize', async () => {
      ...
      await waitFor(() => {
        const addPrizeButton = screen.getByRole('button', {
          name: /add prize/i,
        });
    ...
    it('updates rule content', async () => {
      ...
      await waitFor(() => {
        ...
        fireEvent.change(ruleInput, {
          target: { value: 'Must be 18 or older' },
        });
  ...
  describe('Form Validation', () => {
    it('shows error for empty title', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockToast.error).toHaveBeenCalledWith('Game title is required');
      });
    });
  ...
  describe('Form Submission', () => {
    it('submits form successfully', async () => {
      const { vendorService } = jest.requireActual('@/lib/api/vendorService');
      ...
      await waitFor(() => {
        expect(vendorService.createGame).toHaveBeenCalledWith(
          'user123',
    ...
    it('handles submission error', async () => {
      ...
      await waitFor(() => {
        expect(mockToast.error).toHaveBeenCalledWith('Failed to create game');
      });
    });
  });
  ...
  describe('Draft Saving', () => {
    it('saves draft successfully', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockToast.success).toHaveBeenCalledWith('Game saved as draft');
      });
    });
  });
  ...
  describe('Loading States', () => {
    it('shows loading state during submission', async () => {
      const { vendorService } = jest.requireActual('@/lib/api/vendorService');
      vendorService.createGame.mockImplementation(
