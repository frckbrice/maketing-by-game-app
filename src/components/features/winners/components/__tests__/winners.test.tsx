...
describe('WinnersPage', () => {
  ...
  beforeEach(() => {
    ...
    const { firestoreService } = jest.requireActual('@/lib/firebase/services');
  ...
  it('displays the main title and subtitle', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('Celebrating our lucky winners')
      ).toBeInTheDocument();
    });
  });
  ...
  it('shows no winners message when no data is available', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('Check back later for new winners')
      ).toBeInTheDocument();
    });
  });
  ...
  it('displays winner images correctly', async () => {
    ...
    await waitFor(() => {
      const winnerImages = screen.getAllByAltText(
        /Winner|NGUYEN SOPHIE|JOHN DOE/
      );
