...
export const useCreateOrder = () => {
  ...
  return useMutation({
    mutationFn: async (data: CreateOrderData): Promise<Order> => {
      ...
      if (isDev) {
        const deliveryMethod = mockDeliveryMethods.find(dm => dm.id === data.deliveryMethodId);
        ...
        const newOrder: Order = {
          ...
          createdAt: Date.now(),
          updatedAt: Date.now(),
          estimatedDeliveryDate: Date.now() + (deliveryMethod.estimatedDays * 24 * 60 * 60 * 1000),
    ...
    onSuccess: (newOrder) => {
      queryClient.setQueryData(['orders', { userId: newOrder.userId }], (old: any) => {
...
export const useUpdateOrder = () => {
  ...
  return useMutation({
    mutationFn: async (data: UpdateOrderData): Promise<Order> => {
      ...
      if (isDev) {
        const orderIndex = mockOrders.findIndex(order => order.id === data.orderId);
        if (orderIndex === -1) {
    ...
    onSuccess: (updatedOrder) => {
      ...
      queryClient.setQueryData(['orders', { userId: updatedOrder.userId }], (old: any) => {
        if (!old) return old;
        return {
          ...old,
...
export const useCancelOrder = () => {
  ...
  return useMutation({
    ...
    onSuccess: (cancelledOrder) => {
      ...
      // Update orders list cache
      queryClient.setQueryData(['orders', { userId: cancelledOrder.userId }], (old: any) => {
        if (!old) return old;
        return {
          ...old,
          orders: old.orders.map((order: Order) => 
...
export const useCreateAddress = () => {
  ...
  return useMutation({
    mutationFn: async (addressData: Omit<Address, 'id' | 'createdAt' | 'updatedAt'>): Promise<Address> => {
      await simulateDelay();
      ...
      if (isDev) {
        ...
        const userAddresses = mockAddresses.filter(addr => addr.userId === addressData.userId);
    ...
    onSuccess: (newAddress) => {
      queryClient.setQueryData(['addresses', newAddress.userId], (old: Address[] = []) => {
...
export const useUpdateAddress = () => {
  ...
  return useMutation({
    mutationFn: async ({ addressId, updates }: { 
      addressId: string; 
    ...
    onSuccess: (updatedAddress) => {
      ...
      queryClient.setQueryData(['addresses', updatedAddress.userId], (old: Address[] = []) => {
        return old.map(addr => addr.id === updatedAddress.id ? updatedAddress : addr);
      });
...
export const useDeleteAddress = () => {
  ...
  return useMutation({
    ...
    onSuccess: (deletedAddressId) => {
      ...
      const queries = queryClient.getQueriesData(['addresses']);
      queries.forEach(([key, data]) => {
        if (Array.isArray(data)) {
          const addresses = data as Address[];
          const deletedAddress = addresses.find(addr => addr.id === deletedAddressId);
          if (deletedAddress) {
            queryClient.setQueryData(['addresses', deletedAddress.userId], 
      ...
      queryClient.removeQueries(['address', deletedAddressId]);
