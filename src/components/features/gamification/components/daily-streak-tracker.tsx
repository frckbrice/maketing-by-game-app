...
export const DailyStreakTracker: React.FC = () => {
  ...
  const getStreakMilestone = (days: number) => {
    if (days >= 100) return { icon: Trophy, color: 'text-purple-600', bg: 'bg-purple-100 dark:bg-purple-900/20' };
    if (days >= 30) return { icon: Star, color: 'text-yellow-600', bg: 'bg-yellow-100 dark:bg-yellow-900/20' };
    if (days >= 7) return { icon: Flame, color: 'text-red-600', bg: 'bg-red-100 dark:bg-red-900/20' };
    return { icon: Calendar, color: 'text-blue-600', bg: 'bg-blue-100 dark:bg-blue-900/20' };
  };
  ...
  const milestone = streakData ? getStreakMilestone(streakData.currentStreak) : getStreakMilestone(0);
  const MilestoneIcon = milestone.icon;
  const nextMilestone = streakData ? getNextMilestone(streakData.currentStreak) : 7;
  const today = new Date().toDateString();
  const lastLoginDay = streakData ? new Date(streakData.lastLoginDate).toDateString() : null;
        ...
        {streakData ? (
          <div className="space-y-6">
            ...
            <div className="text-center space-y-2">
              <div className={`inline-flex items-center justify-center w-20 h-20 ${milestone.bg} rounded-full`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div className="text-xl font-bold text-orange-600">{streakData.longestStreak}</div>
              ...
              {isCheckedInToday ? (
                <div className="space-y-2">
                  <div className="flex items-center justify-center space-x-2 text-green-600">
                    ...
                    <span className="font-medium">{t('gamification.streak.checkedInToday')}</span>
                  </div>
            ...
            <div className="space-y-3">
              <h4 className="font-medium text-sm">{t('gamification.streak.milestones')}</h4>
              <div className="space-y-2">
            ...
            {/* Recent Bonuses */}
            {streakData.streakBonuses && streakData.streakBonuses.length > 0 && (
              <div className="space-y-3">
                <h4 className="font-medium text-sm">{t('gamification.streak.recentBonuses')}</h4>
          ...
          <div className="text-center space-y-4">
            ...
            <div>
              <h3 className="text-lg font-semibold mb-2">{t('gamification.streak.getStarted')}</h3>
