...
import { Award, Star, TrendingUp, Users, Gamepad2, ShoppingBag } from 'lucide-react';
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              {checkBadges.isPending ? t('common.checking') : t('gamification.badges.checkNew')}
            </Button>
          </div>
        </CardTitle>
      </CardHeader>
        ...
        {userBadges.length > 0 ? (
          <div className="space-y-6">
            {Object.entries(groupedBadges).map(([category, categoryBadges]) => {
              const CategoryIcon = CATEGORY_ICONS[category as keyof typeof CATEGORY_ICONS] || Award;
              const categoryColor = CATEGORY_COLORS[category as keyof typeof CATEGORY_COLORS] || 'text-gray-600 bg-gray-100';
                ...
                <div key={category} className="space-y-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <h3 className="font-medium text-lg">
                      {t(`gamification.badges.categories.${category.toLowerCase()}`)}
                    </h3>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {categoryBadges
                      ...
                      >
                        {/* Badge Icon/Image */}
                        <div className={`w-16 h-16 mx-auto mb-3 rounded-full flex items-center justify-center ${categoryColor}`}>
                          {badge.badgeDetails?.icon ? (
                            <span className="text-2xl">{badge.badgeDetails.icon}</span>
                          ) : (
                        ...
                        <div className="text-center space-y-2">
                          <h4 className="font-medium text-sm line-clamp-2">
                            {badge.badgeDetails?.name || t('gamification.badges.unknownBadge')}
                          </h4>
                          ...
                          {badge.badgeDetails?.rewardPoints && (
                            <Badge variant="secondary" className="text-xs">
                              +{badge.badgeDetails.rewardPoints} {t('gamification.points')}
                            </Badge>
                          )}
                        ...
                        {badge.badgeDetails?.type === 'PROGRESS' && badge.progress !== undefined && (
                          <div className="mt-2">
            ...
            <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <h3 className="font-medium mb-3">{t('gamification.badges.statistics')}</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-blue-600">{userBadges.length}</div>
                  <div className="text-xs text-gray-600 dark:text-gray-400">{t('gamification.badges.totalEarned')}</div>
                </div>
                
                <div>
                  <div className="text-2xl font-bold text-green-600">
                    {userBadges.filter(b => b.badgeDetails?.type === 'MILESTONE').length}
                  </div>
                  <div className="text-xs text-gray-600 dark:text-gray-400">{t('gamification.badges.milestones')}</div>
                </div>
                
                <div>
                  <div className="text-2xl font-bold text-purple-600">
                    {userBadges.filter(b => b.badgeDetails?.type === 'SPECIAL').length}
                  </div>
                  <div className="text-xs text-gray-600 dark:text-gray-400">{t('gamification.badges.special')}</div>
                </div>
                
                <div>
                  <div className="text-2xl font-bold text-orange-600">
                    {userBadges.reduce((sum, b) => sum + (b.badgeDetails?.rewardPoints || 0), 0)}
                  </div>
                  <div className="text-xs text-gray-600 dark:text-gray-400">{t('gamification.badges.pointsFromBadges')}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="text-center space-y-4 py-8">
            ...
            <div>
              <h3 className="text-lg font-semibold mb-2">{t('gamification.badges.noBadges')}</h3>
