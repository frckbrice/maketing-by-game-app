      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-5">
          <TabsTrigger value="loyalty" className="flex items-center space-x-2">
            ...
            <span className="hidden sm:inline">{t('gamification.tabs.loyalty')}</span>
          </TabsTrigger>
          <TabsTrigger value="referrals" className="flex items-center space-x-2">
            <Users className="w-4 h-4" />
            <span className="hidden sm:inline">{t('gamification.tabs.referrals')}</span>
          </TabsTrigger>
          <TabsTrigger value="streak" className="flex items-center space-x-2">
            <Calendar className="w-4 h-4" />
            <span className="hidden sm:inline">{t('gamification.tabs.streak')}</span>
          </TabsTrigger>
          <TabsTrigger value="badges" className="flex items-center space-x-2">
            <Award className="w-4 h-4" />
            <span className="hidden sm:inline">{t('gamification.tabs.badges')}</span>
          </TabsTrigger>
          <TabsTrigger value="notifications" className="flex items-center space-x-2">
            <Bell className="w-4 h-4" />
            <span className="hidden sm:inline">{t('gamification.tabs.notifications')}</span>
          </TabsTrigger>
        </TabsList>
