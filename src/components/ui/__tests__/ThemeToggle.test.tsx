...
jest.mock('../Button', () => ({
    Button: ({ children, onClick, variant, size, className, 'aria-label': ariaLabel }: any) => (
        <button onClick={onClick} className={className} data-variant={variant} data-size={size} aria-label={ariaLabel}>
            {children}
        </button>
    ),
}));
...
describe('ThemeToggle', () => {
    ...
    it('renders loading spinner when not mounted', () => {
        ...
        if (loadingSpinner) {
            ...
            expect(loadingSpinner).toHaveClass('w-4', 'h-4', 'bg-current', 'rounded-full', 'animate-pulse');
        } else {
