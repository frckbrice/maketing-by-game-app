...
describe('UserProfileDropdown', () => {
  ...
  describe('Role-based Navigation', () => {
    describe('USER Role', () => {
      ...
      it('navigates to games page when dashboard is clicked (users)', () => {
        ...
        const { useAuth } = jest.requireActual('@/lib/contexts/AuthContext');
        const { user } = useAuth();
  ...
  describe('Icons', () => {
    it('displays correct icons for each menu item', () => {
      ...
      const gamepadIcon = document.querySelector(
        'svg[class*="lucide-gamepad-2"]'
      );
    ...
    it('displays admin icons for admin users', () => {
      ...
      const barChartIcon = document.querySelector(
        'svg[class*="lucide-bar-chart-3"]'
      );
  ...
  describe('Error Handling', () => {
    it('handles logout errors gracefully', async () => {
      const mockUser = createMockUser('USER');
      const mockLogoutWithError = jest
        .fn()
