...
describe('TrustBadge', () => {
  ...
  it('handles long text', () => {
    const longText =
      'This is a very long trust badge text that might wrap to multiple lines';
  ...
  it('handles multiple trust badges with different colors', () => {
    const { rerender } = render(
      <TrustBadge {...defaultProps} iconColor='text-green-500' />
    );
  ...
  it('renders with different icon types', () => {
    ...
    icons.forEach(IconComponent => {
      const { unmount } = render(
        <TrustBadge {...defaultProps} icon={IconComponent} />
      );
