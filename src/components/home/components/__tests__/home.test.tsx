...
jest.mock('../MobileNavigation', () => ({
    MobileNavigation: ({ isDark, mobileMenuOpen, setMobileMenuOpen, onThemeToggle }: any) => (
        <div data-testid="mobile-navigation" data-is-dark={isDark} data-menu-open={mobileMenuOpen}>
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>Toggle Menu</button>
            <button onClick={onThemeToggle}>Toggle Theme</button>
        </div>
    ),
}));
...
describe('HomePageComponent', () => {
    ...
    it('applies dark theme styling when isDark is true', async () => {
        ...
        await waitFor(() => {
            ...
            expect(mainContainer).toHaveClass('bg-gradient-to-br', 'from-gray-900', 'via-black', 'to-gray-900');
    ...
    it('renders in correct order', async () => {
        ...
        await waitFor(() => {
            ...
            expect(children[3]).toHaveAttribute('data-testid', 'how-it-works-section');
    ...
    it('handles theme toggle through header', async () => {
        ...
        await waitFor(() => {
            const themeToggleButton = screen.getByTestId('desktop-header').querySelector('button');
