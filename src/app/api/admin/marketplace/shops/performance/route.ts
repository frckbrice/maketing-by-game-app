...
import {
  Shop,
  ShopsPerformanceData,
} from '../../../../../../components/features/shops/api/types';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const shopPerformance: ShopsPerformanceData[] = shops.map((shop: Shop) => {
      ...
      const likes = shopProducts.reduce(
        (sum: number, product: Product) => sum + (product.likeCount || 0),
        0
      );
      ...
      const conversionRate =
        engagement > 0 ? (orderCount / engagement) * 100 : 0;
    ...
    const topShops = shopPerformance
      .sort(
        (a: ShopsPerformanceData, b: ShopsPerformanceData) =>
          b.revenue - a.revenue
      )
      .slice(0, limit);
    ...
    if (
      ...
      error.message.includes('Failed to fetch required data')
    ) {
      return NextResponse.json(
        {
          success: false,
