...
export async function GET(request: NextRequest) {
  try {
    ...
    const productPerformance: ProductPerformanceData[] = products.map(
      (product: Product) => {
        ...
        const views = product.reviewsCount || 0;
        const conversionRate = views > 0 ? (sales / views) * 100 : 0;

        return {
          productId: product.id,
          productName: product.name,
          shopName: shopMap.get(product.shop.id as string) || 'Unknown Shop',
          sales,
          revenue,
    ...
    const topProducts = productPerformance
      .sort(
        (a: ProductPerformanceData, b: ProductPerformanceData) =>
          b.revenue - a.revenue
      )
      .slice(0, limit);
