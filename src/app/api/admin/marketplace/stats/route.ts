...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalFollows = shops.reduce(
      (sum: number, shop: Shop) => sum + (shop.followersCount || 0),
      0
    );
    const totalReviews = shops.reduce(
      (sum: number, shop: Shop) => sum + (shop.reviewsCount || 0),
      0
    );
    ...
    const averageShopRating =
      shops.length > 0
        ? shops.reduce(
            (sum: number, shop: Shop) => sum + (shop.rating || 0),
            0
          ) / shops.length
        : 0;

    const activeShops = shops.filter(
      (shop: Shop) => shop.status === 'active'
    ).length;
