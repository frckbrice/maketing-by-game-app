...
export async function GET(request: NextRequest) {
  try {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const views = dayProducts.reduce(
        (sum: number, product: Product) => sum + (product.reviewsCount || 0),
        0
      );
      const likes = dayProducts.reduce(
    ...
    const productPerformance = products
      .map((product: Product) => {
        ...
        return {
          ...
          views: product.reviewsCount || 0,
          rating: product.rating || 0,
          conversionRate:
            (product.reviewsCount || 0) > 0
              ? (sales / (product.reviewsCount || 1)) * 100
              : 0,
        };
      })
