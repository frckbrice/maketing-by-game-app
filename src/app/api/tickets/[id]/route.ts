...
export async function GET(
  ...
  try {
    ...
    try {
      // Get game details
      const gameDoc = await adminFirestore.collection('games').doc(ticket.gameId).get();
      if (gameDoc.exists) {
        gameDetails = {
          id: gameDoc.id,
      ...
      // Get vendor details
      const vendorDoc = await adminFirestore.collection('users').doc(ticket.vendorId).get();
      if (vendorDoc.exists) {
        vendorDetails = {
          id: vendorDoc.id,
    ...
    if (user.role === 'ADMIN' || (user.role === 'VENDOR' && ticket.vendorId === user.id)) {
      try {
