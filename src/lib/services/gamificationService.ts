...
class GamificationService {
  ...
  async getUserLoyaltyProfile(
    userId: string
  ): Promise<UserLoyaltyProfile | null> {
    try {
      ...
      return { id: doc.id, ...doc.data() } as unknown as UserLoyaltyProfile;
    } catch (error) {
  ...
  async createDefaultLoyaltyProfile(
    userId: string
  ...
  async awardPoints(
    ...
    try {
      ...
      const transactionRef = adminFirestore
        .collection('loyaltyPointTransactions')
        .doc();
      ...
      let profile = profileDoc.exists
        ? ({
            id: profileDoc.id,
            ...profileDoc.data(),
          } as unknown as UserLoyaltyProfile)
      ...
      const updatedTotalSpent =
        points < 0
          ? profile.totalPointsSpent + Math.abs(points)
          : profile.totalPointsSpent;
      ...
      if (leveledUp) {
        const levelNotificationRef = adminFirestore
          .collection('gamificationNotifications')
          .doc();
  ...
  async calculateUserLevel(
    totalPoints: number
  ): Promise<{ level: number; name: string; nextThreshold: number }> {
    const levels = [
      ...
      { level: 6, name: 'Elite', minPoints: 25000, nextThreshold: 50000 },
      {
        ...
        name: 'Legend',
        minPoints: 50000,
        nextThreshold: Number.MAX_SAFE_INTEGER,
      },
    ];
  ...
  async updateDailyStreak(
    userId: string
  ): Promise<{ streakUpdated: boolean; pointsAwarded: number }> {
    try {
      ...
      if (!streakDoc.exists) {
        ...
        if (lastLoginDay === yesterday) {
          ...
          pointsAwarded = await this.awardStreakBonus(
            userId,
            streak.currentStreak
          );
        } else {
  ...
  private async completeReferral(referralId: string): Promise<void> {
    try {
      const referralRef = adminFirestore
        .collection('referrals')
        .doc(referralId);
      ...
      await adminFirestore
        ...
        .update({
          ...
          pendingReferrals: FieldValue.increment(-1),
          totalRewardsEarned: FieldValue.increment(300),
          updatedAt: Date.now(),
        });
  ...
  async checkAndAwardBadges(userId: string): Promise<UserBadge[]> {
    try {
      ...
      for (const badge of badges) {
        ...
        // Check requirements
        if (
          badge.requirements.gamesPlayed &&
          loyaltyProfile.lifetimeGamesPlayed >= badge.requirements.gamesPlayed
        ) {
          qualified = true;
        }

        if (
          badge.requirements.totalSpent &&
          loyaltyProfile.totalPointsEarned >= badge.requirements.totalSpent
        ) {
          qualified = true;
        }
        ...
        if (badge.requirements.referrals) {
          ...
          if (
            referralProfile &&
            referralProfile.successfulReferrals >= badge.requirements.referrals
          ) {
            qualified = true;
          }
        }
        ...
        if (badge.requirements.consecutiveDays) {
          ...
          if (
            streak &&
            streak.longestStreak >= badge.requirements.consecutiveDays
          ) {
            qualified = true;
          }
        }
  ...
  // Helper methods
  async getUserReferralProfile(
    userId: string
  ): Promise<UserReferralProfile | null> {
    try {
      const doc = await adminFirestore
        .collection('userReferralProfiles')
        .doc(userId)
      ...
      return doc.exists
        ? ({ id: doc.id, ...doc.data() } as unknown as UserReferralProfile)
        : null;
    } catch (error) {
  ...
  async getUserDailyStreak(userId: string): Promise<DailyStreak | null> {
    try {
      ...
      return doc.exists
        ? ({ id: doc.id, ...doc.data() } as unknown as DailyStreak)
        : null;
    } catch (error) {
